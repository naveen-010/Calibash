...
file_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(os.getcwd(), "events.csv")
            ...
            #     print(f'\033[38;5;230m{simv}\033[0m')
                ...
                while True:
                    ...
                    if x == 'q' or x == 'quit' or x == 'exit':
                        ...
                        1. General Behavior:
                           - Return a python dictionary as output, "NOTHING ELSE, NOT EVEN A SINGLE WORD should be returned other than the dictionary."
                           - The first key in the dictionary must always be 'event', and its value should be one of the following based on the user’s intent:
                             - 'add': If the user mentions adding, creating, or scheduling an event. If the user doesn’t explicitly mention they want to add something as an event but their description fits the definition of an event(for eg. "Tomorrow i am going for swimming."), use your judgment to add it anyway. Take today's date for reference as {today}.
                             ...
                             - 'others': If user talks about anything unrelated to add, edit, view or remove.
                           - The second key in dictionary should be 'message' which will be a string (text) message.
                           - Add more keys as required by different events(like add , edit etc).

                        2. Add Event:
                           - If the user wants to add an event , then add 6 keys ({fieldnames}), extract the value of keys from their input:
                             ...
                             - 'Category': Assign a category yourself (if not mentioned by user) based on what the Title is (like cycling is personal while meeting is work).
                           - In the value of message key , add a detailed message that tells details like meeting scheduled for 8am or whatever based on user input.

                        3. Edit Event:
                           - If the user wants to edit an event , then add 3 keys ('Title', 'Attribute', 'New_value'), extract the value of keys from their input:
                             - 'Title': The name of the event they want to edit. Match it to the closest title from the list of available titles {titles}. If the user enters something like "car" match it to "Car Racing" or "Car Tournament", depending on what exists in {titles}. If no match is found, return with a message that says the event is not present.
                             ...
                             - 'New_value': The new value they want to assign to the chosen attribute.If attribute is date, New_value should be in DD-MM-YYY format, if attribute is time , New_value should be in 24 hrs system and if attriubte is duration, it should be in minutes. 
                           - In the value of message key , add a detailed message that tells meetings time has been updated to 9 am or whatever based on user input.
                           - Example - user : change exercise time to 9 am. Values extracted - Title : Exercise, New_value : 9:00, Attribute: Time. 
                           - Example - user : change duration of meeting to 2 hr. Values extracted - Title : Meeting, New_value = 120.
                           - Example - user : change swimming category to fun. Values extracted - Title : Swimming, New_value = Fun, Attribute : Category. 

                        4. Remove Event:
                           - If the user wants to remove or delete an event,add one key ('Title') and extract its value from user input:
                           ...
                           - Match the title to the closest one from {titles}. If the user types something like "car" match it to "Car Racing" or "Car Tournament" depending on what exists in {titles}. If no match is found, return with a message that says the event is not present else return with message that event is removed.

                        5. View Event:
                           - If the user wants to view an event (mentions terms like "show", "see", "display", etc.), just set the value of the 'event' key to 'view'. Set message key as 'viewing events'.
                        6. Others:
                           - In the value of message key, add a message roasting the user.Roast them with maximum savagery and creativity. No mercy should be shown. If they’re discussing sensitive topics like depression or making self-destructive statements, respond with brutal sarcasm, as many users ask these things for fun. If the user says sensitive like "I'm thinking about giving up on everything." respond with something like: "Wow, deep thoughts for someone who can’t even schedule a simple event." However, if no roast-worthy material exists, don’t force it. Skip the roast.

                        """
                        out = get_completion(mainprompt, x)
                        ...
                        #  print(f'\033[38;5;85m{out}\033[0m')
                        try:
                            out = json.loads(out)
                            if out["event"] == "add":
                                t, l, dur, st, c, d = (
                            ...
                            if out['event'] != 'view':
                                if  out['event'] != 'others':
                                    print(f'\033[38;5;85m{out['message']}\033[0m')
                                    view_event()
                                else:
                                    print(f'\033[38;5;85m{out['message']}\033[0m')
